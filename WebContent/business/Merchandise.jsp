...
<jsp:include page="top.jsp"></jsp:include>
		...
		<div class="span9">
			<div class="well summary">
				<ul>
					...
					<li  class="active"><span>仓库中的商品</span></li>
				</ul>
			</div>
			<div class="sell">
					<div class="row-fluid">
	...
	function find1() {
		$.ajax({
				type:"get",
				url:"${pageContext.request.contextPath }/business/findSellMer.action?i="+0,
				async: false,
				...
				success:function(data){
					...
					$(data).each(
						function(){
							...
							let td_button = $("<td><input type='button' class='view-link' value='上架' gId="+this.gId+" onclick='up(this)'>"+
									"<input type='button' class='view-link' value='删除' gId="+this.gId+" onclick='deleteGood(this)'></td></tr>");
	...
	function up(th){
		var con=confirm("确认上架该商品？"); 
		if(con==true)
			window.location.href="../order/upGood.action?gId="+th.getAttribute("gId");
	}
	function deleteGood(th){
		var con=confirm("确认删除该商品？"); 
		if(con==true)
			window.location.href="../order/deleteGood.action?gId="+th.getAttribute("gId");
	}
	</script>
