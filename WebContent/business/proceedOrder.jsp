	...
	<div class="row">
				<div class="span3">
					<div class="well" style="padding: 8px 0;">
						<ul class="nav nav-list">
							...
							<li  class="active">
								<a href="proceedOrder.jsp"><i class="icon-white icon-check"></i>进行中的订单</a>
							</li>
							<li>
			...
			<!-- ------------订单部分一致的地方------------------- -->
				<div class="takeButton">
					<input type="button" value="全部不完成" onclick="allNotComplete()">
					<input type="button" value="全部完成" onclick="allComplete()">
				</div>
				<hr>
				<div class="order">
<!-- 					订单号  收货人  商品 总价  备注信息  收货地址  下单时间  支付状态  操作 -->
	...
	function find1() {
		$.ajax({
				type:"get",
				url:"${pageContext.request.contextPath }/order/findProceedOrder.action",
				async: false,
					...
					$(data).each(
						function(){
							...
							$(orderRight).append.call($(orderRight),
									...
									span_state);
							let button=$("<div class='takeButton'><input type='button' value='未完成' class='notTake'  oId="+this.orders.oId+" onclick='notTake(this)'>"
									+"<input type='button' value='完成' class='take'  oId="+this.orders.oId+" onclick='take(this)'><div><hr>");
	...
	function notTake(th){
		window.location.href="../order/notComplete.action?oId="+th.getAttribute("oid");
	}
	function take(th){
		window.location.href="../order/complete.action?oId="+th.getAttribute("oid");
	}
	function allComplete(){
		window.location.href="../order/allComplete.action";
	}
	function allNotComplete(){
		window.location.href="../order/allNotComplete.action";
	}
	</script>
